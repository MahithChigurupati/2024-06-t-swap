...
contract Handler is Test {
    ...
    function deposit(uint256 wethToDeposit) public {
        ...
        startingWeth = weth.balanceOf(liquidityProvider);
        startingToken = token.balanceOf(liquidityProvider);
        ...
        pool.deposit(wethToDeposit, poolTokenToDeposit, wethToDeposit, uint64(block.timestamp));
        vm.stopPrank();

        actualDeltaWeth = weth.balanceOf(liquidityProvider) - startingWeth;
        actualDeltaToken = token.balanceOf(liquidityProvider) - startingToken;
    }
